...
static int service_registry_path(const TCHAR *service_name, bool parameters, const TCHAR *sub, TCHAR *buffer, unsigned long buflen) {
  ...
  if (parameters) {
    if (sub) ret = _sntprintf_s(buffer, buflen, _TRUNCATE, FCSM_REGISTRY _T("\\") FCSM_REG_PARAMETERS _T("\\%s"), service_name, sub);
    else ret = _sntprintf_s(buffer, buflen, _TRUNCATE, FCSM_REGISTRY _T("\\") FCSM_REG_PARAMETERS, service_name);
  }
  else ret = _sntprintf_s(buffer, buflen, _TRUNCATE, FCSM_REGISTRY, service_name);
...
static long open_registry_key(const TCHAR *registry, REGSAM sam, HKEY *key, bool must_exist) {
  ...
  if (sam & KEY_SET_VALUE) {
    ...
    if (error != ERROR_SUCCESS) {
      *key = 0;
      log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OPENKEY_FAILED, registry, error_string(GetLastError()), 0);
      return error;
    }
  }
  else {
    ...
    if (error != ERROR_SUCCESS) {
      *key = 0;
      if (error != ERROR_FILE_NOT_FOUND || must_exist) log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OPENKEY_FAILED, registry, error_string(GetLastError()), 0);
    }
  }
...
int create_messages() {
  ...
  if (_sntprintf_s(registry, _countof(registry), _TRUNCATE, _T("SYSTEM\\CurrentControlSet\\Services\\EventLog\\Application\\%s"), FCSM) < 0) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, _T("eventlog registry"), _T("create_messages()"), 0);
    return 1;
  }
  ...
  const TCHAR *path = fcsm_unquoted_imagepath();
...
int create_parameters(fcsm_service_t *service, bool editing) {
  ...
  if (set_expand_string(key, FCSM_REG_EXE, service->exe)) {
    if (ret > 0) RegDeleteKey(HKEY_LOCAL_MACHINE, registry);
    RegCloseKey(key);
    return 2;
  }
  if (set_expand_string(key, FCSM_REG_FLAGS, service->flags)) {
    if (ret > 0) RegDeleteKey(HKEY_LOCAL_MACHINE, registry);
    RegCloseKey(key);
    return 3;
  }
  if (set_expand_string(key, FCSM_REG_DIR, service->dir)) {
    if (ret > 0) RegDeleteKey(HKEY_LOCAL_MACHINE, registry);
    RegCloseKey(key);
    return 4;
  }
  ...
  if (service->priority != NORMAL_PRIORITY_CLASS) set_number(key, FCSM_REG_PRIORITY, service->priority);
  else if (editing) RegDeleteValue(key, FCSM_REG_PRIORITY);
  if (service->affinity) {
    TCHAR *string;
    if (! affinity_mask_to_string(service->affinity, &string)) {
      if (RegSetValueEx(key, FCSM_REG_AFFINITY, 0, REG_SZ, (const unsigned char *) string, (unsigned long) (_tcslen(string) + 1) * sizeof(TCHAR)) != ERROR_SUCCESS) {
        log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_SETVALUE_FAILED, FCSM_REG_AFFINITY, error_string(GetLastError()), 0);
  ...
  else if (editing) RegDeleteValue(key, FCSM_REG_AFFINITY);
  unsigned long stop_method_skip = ~service->stop_method;
  if (stop_method_skip) set_number(key, FCSM_REG_STOP_METHOD_SKIP, stop_method_skip);
  else if (editing) RegDeleteValue(key, FCSM_REG_STOP_METHOD_SKIP);
  if (service->default_exit_action < FCSM_NUM_EXIT_ACTIONS) create_exit_action(service->name, exit_action_strings[service->default_exit_action], editing);
  if (service->restart_delay) set_number(key, FCSM_REG_RESTART_DELAY, service->restart_delay);
  else if (editing) RegDeleteValue(key, FCSM_REG_RESTART_DELAY);
  if (service->throttle_delay != FCSM_RESET_THROTTLE_RESTART) set_number(key, FCSM_REG_THROTTLE, service->throttle_delay);
  else if (editing) RegDeleteValue(key, FCSM_REG_THROTTLE);
  if (service->kill_console_delay != FCSM_KILL_CONSOLE_GRACE_PERIOD) set_number(key, FCSM_REG_KILL_CONSOLE_GRACE_PERIOD, service->kill_console_delay);
  else if (editing) RegDeleteValue(key, FCSM_REG_KILL_CONSOLE_GRACE_PERIOD);
  if (service->kill_window_delay != FCSM_KILL_WINDOW_GRACE_PERIOD) set_number(key, FCSM_REG_KILL_WINDOW_GRACE_PERIOD, service->kill_window_delay);
  else if (editing) RegDeleteValue(key, FCSM_REG_KILL_WINDOW_GRACE_PERIOD);
  if (service->kill_threads_delay != FCSM_KILL_THREADS_GRACE_PERIOD) set_number(key, FCSM_REG_KILL_THREADS_GRACE_PERIOD, service->kill_threads_delay);
  else if (editing) RegDeleteValue(key, FCSM_REG_KILL_THREADS_GRACE_PERIOD);
  if (! service->kill_process_tree) set_number(key, FCSM_REG_KILL_PROCESS_TREE, 0);
  else if (editing) RegDeleteValue(key, FCSM_REG_KILL_PROCESS_TREE);
  if (service->stdin_path[0] || editing) {
    if (service->stdin_path[0]) set_expand_string(key, FCSM_REG_STDIN, service->stdin_path);
    else if (editing) RegDeleteValue(key, FCSM_REG_STDIN);
    if (service->stdin_sharing != FCSM_STDIN_SHARING) set_createfile_parameter(key, FCSM_REG_STDIN, FCSM_REG_STDIO_SHARING, service->stdin_sharing);
    else if (editing) delete_createfile_parameter(key, FCSM_REG_STDIN, FCSM_REG_STDIO_SHARING);
    if (service->stdin_disposition != FCSM_STDIN_DISPOSITION) set_createfile_parameter(key, FCSM_REG_STDIN, FCSM_REG_STDIO_DISPOSITION, service->stdin_disposition);
    else if (editing) delete_createfile_parameter(key, FCSM_REG_STDIN, FCSM_REG_STDIO_DISPOSITION);
    if (service->stdin_flags != FCSM_STDIN_FLAGS) set_createfile_parameter(key, FCSM_REG_STDIN, FCSM_REG_STDIO_FLAGS, service->stdin_flags);
    else if (editing) delete_createfile_parameter(key, FCSM_REG_STDIN, FCSM_REG_STDIO_FLAGS);
  }
  if (service->stdout_path[0] || editing) {
    if (service->stdout_path[0]) set_expand_string(key, FCSM_REG_STDOUT, service->stdout_path);
    else if (editing) RegDeleteValue(key, FCSM_REG_STDOUT);
    if (service->stdout_sharing != FCSM_STDOUT_SHARING) set_createfile_parameter(key, FCSM_REG_STDOUT, FCSM_REG_STDIO_SHARING, service->stdout_sharing);
    else if (editing) delete_createfile_parameter(key, FCSM_REG_STDOUT, FCSM_REG_STDIO_SHARING);
    if (service->stdout_disposition != FCSM_STDOUT_DISPOSITION) set_createfile_parameter(key, FCSM_REG_STDOUT, FCSM_REG_STDIO_DISPOSITION, service->stdout_disposition);
    else if (editing) delete_createfile_parameter(key, FCSM_REG_STDOUT, FCSM_REG_STDIO_DISPOSITION);
    if (service->stdout_flags != FCSM_STDOUT_FLAGS) set_createfile_parameter(key, FCSM_REG_STDOUT, FCSM_REG_STDIO_FLAGS, service->stdout_flags);
    else if (editing) delete_createfile_parameter(key, FCSM_REG_STDOUT, FCSM_REG_STDIO_FLAGS);
    if (service->stdout_copy_and_truncate) set_createfile_parameter(key, FCSM_REG_STDOUT, FCSM_REG_STDIO_COPY_AND_TRUNCATE, 1);
    else if (editing) delete_createfile_parameter(key, FCSM_REG_STDOUT, FCSM_REG_STDIO_COPY_AND_TRUNCATE);
  }
  if (service->stderr_path[0] || editing) {
    if (service->stderr_path[0]) set_expand_string(key, FCSM_REG_STDERR, service->stderr_path);
    else if (editing) RegDeleteValue(key, FCSM_REG_STDERR);
    if (service->stderr_sharing != FCSM_STDERR_SHARING) set_createfile_parameter(key, FCSM_REG_STDERR, FCSM_REG_STDIO_SHARING, service->stderr_sharing);
    else if (editing) delete_createfile_parameter(key, FCSM_REG_STDERR, FCSM_REG_STDIO_SHARING);
    if (service->stderr_disposition != FCSM_STDERR_DISPOSITION) set_createfile_parameter(key, FCSM_REG_STDERR, FCSM_REG_STDIO_DISPOSITION, service->stderr_disposition);
    else if (editing) delete_createfile_parameter(key, FCSM_REG_STDERR, FCSM_REG_STDIO_DISPOSITION);
    if (service->stderr_flags != FCSM_STDERR_FLAGS) set_createfile_parameter(key, FCSM_REG_STDERR, FCSM_REG_STDIO_FLAGS, service->stderr_flags);
    else if (editing) delete_createfile_parameter(key, FCSM_REG_STDERR, FCSM_REG_STDIO_FLAGS);
    if (service->stderr_copy_and_truncate) set_createfile_parameter(key, FCSM_REG_STDERR, FCSM_REG_STDIO_COPY_AND_TRUNCATE, 1);
    else if (editing) delete_createfile_parameter(key, FCSM_REG_STDERR, FCSM_REG_STDIO_COPY_AND_TRUNCATE);
  }
  if (service->timestamp_log) set_number(key, FCSM_REG_TIMESTAMP_LOG, 1);
  else if (editing) RegDeleteValue(key, FCSM_REG_TIMESTAMP_LOG);
  if (service->hook_share_output_handles) set_number(key, FCSM_REG_HOOK_SHARE_OUTPUT_HANDLES, 1);
  else if (editing) RegDeleteValue(key, FCSM_REG_HOOK_SHARE_OUTPUT_HANDLES);
  if (service->rotate_files) set_number(key, FCSM_REG_ROTATE, 1);
  else if (editing) RegDeleteValue(key, FCSM_REG_ROTATE);
  if (service->rotate_stdout_online) set_number(key, FCSM_REG_ROTATE_ONLINE, 1);
  else if (editing) RegDeleteValue(key, FCSM_REG_ROTATE_ONLINE);
  if (service->rotate_seconds) set_number(key, FCSM_REG_ROTATE_SECONDS, service->rotate_seconds);
  else if (editing) RegDeleteValue(key, FCSM_REG_ROTATE_SECONDS);
  if (service->rotate_bytes_low) set_number(key, FCSM_REG_ROTATE_BYTES_LOW, service->rotate_bytes_low);
  else if (editing) RegDeleteValue(key, FCSM_REG_ROTATE_BYTES_LOW);
  if (service->rotate_bytes_high) set_number(key, FCSM_REG_ROTATE_BYTES_HIGH, service->rotate_bytes_high);
  else if (editing) RegDeleteValue(key, FCSM_REG_ROTATE_BYTES_HIGH);
  if (service->rotate_delay != FCSM_ROTATE_DELAY) set_number(key, FCSM_REG_ROTATE_DELAY, service->rotate_delay);
  else if (editing) RegDeleteValue(key, FCSM_REG_ROTATE_DELAY);
  if (service->no_console) set_number(key, FCSM_REG_NO_CONSOLE, 1);
  else if (editing) RegDeleteValue(key, FCSM_REG_NO_CONSOLE);

  /* Environment */
  if (service->env) {
    if (RegSetValueEx(key, FCSM_REG_ENV, 0, REG_MULTI_SZ, (const unsigned char *) service->env, (unsigned long) service->envlen * sizeof(TCHAR)) != ERROR_SUCCESS) {
      log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_SETVALUE_FAILED, FCSM_REG_ENV, error_string(GetLastError()), 0);
    }
  }
  else if (editing) RegDeleteValue(key, FCSM_REG_ENV);
  if (service->env_extra) {
    if (RegSetValueEx(key, FCSM_REG_ENV_EXTRA, 0, REG_MULTI_SZ, (const unsigned char *) service->env_extra, (unsigned long) service->env_extralen * sizeof(TCHAR)) != ERROR_SUCCESS) {
      log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_SETVALUE_FAILED, FCSM_REG_ENV_EXTRA, error_string(GetLastError()), 0);
    }
  }
  else if (editing) RegDeleteValue(key, FCSM_REG_ENV_EXTRA);
...
int create_exit_action(TCHAR *service_name, const TCHAR *action_string, bool editing) {
  /* Get registry */
  TCHAR registry[KEY_LENGTH];
  if (service_registry_path(service_name, true, FCSM_REG_EXIT, registry, _countof(registry)) < 0) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, _T("FCSM_REG_EXIT"), _T("create_exit_action()"), 0);
    return 1;
  }
  ...
  if (RegSetValueEx(key, 0, 0, REG_SZ, (const unsigned char *) action_string, (unsigned long) (_tcslen(action_string) + 1) * sizeof(TCHAR)) != ERROR_SUCCESS) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_SETVALUE_FAILED, FCSM_REG_EXIT, error_string(GetLastError()), 0);
    RegCloseKey(key);
    return 3;
  }
...
int get_environment(TCHAR *service_name, HKEY key, TCHAR *value, TCHAR **env, unsigned long *envlen) {
  ...
  *envlen = 0;
  ...
  if (ret != ERROR_SUCCESS) {
    *env = 0;
    ...
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_QUERYVALUE_FAILED, value, error_string(ret), 0);
    return 1;
  }

  if (type != REG_MULTI_SZ) {
    log_event(EVENTLOG_WARNING_TYPE, FCSM_EVENT_INVALID_ENVIRONMENT_STRING_TYPE, value, service_name, 0);
    *env = 0;
    return 2;
  }
  ...
  *env = (TCHAR *) HeapAlloc(GetProcessHeap(), 0, envsize);
  if (! *env) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, value, _T("get_environment()"), 0);
    return 4;
  }
...
int get_string(HKEY key, TCHAR *value, TCHAR *data, unsigned long datalen, bool expand, bool sanitise, bool must_exist) {
  ...
  if (! buffer) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, value, _T("get_string()"), 0);
    return 1;
  }
  ...
  if (! ret || ret > datalen) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_EXPANDENVIRONMENTSTRINGS_FAILED, buffer, error_string(GetLastError()), 0);
    HeapFree(GetProcessHeap(), 0, buffer);
    return 3;
  }
...
int set_string(HKEY key, TCHAR *value, TCHAR *string, bool expand) {
  ...
  log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_SETVALUE_FAILED, value, error_string(GetLastError()), 0);
  return 1;
}
...
int copy_double_null(TCHAR *dn, unsigned long dnlen, TCHAR **newdn) {
  ...
  *newdn = (TCHAR *) HeapAlloc(GetProcessHeap(), 0, dnlen * sizeof(TCHAR));
  if (! *newdn) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, _T("dn"), _T("copy_double_null()"), 0);
    return 2;
  }
...
int append_to_double_null(TCHAR *dn, unsigned long dnlen, TCHAR **newdn, unsigned long *newlen, TCHAR *append, size_t keylen, bool case_sensitive) {
  ...
  if (! key) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, _T("key"), _T("append_to_double_null()"), 0);
    return 1;
  }
  ...
  *newdn = (TCHAR *) HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, len * sizeof(TCHAR));
  if (! *newdn) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, _T("newdn"), _T("append_to_double_null()"), 0);
...
int remove_from_double_null(TCHAR *dn, unsigned long dnlen, TCHAR **newdn, unsigned long *newlen, TCHAR *remove, size_t keylen, bool case_sensitive) {
  ...
  key = (TCHAR *) HeapAlloc(GetProcessHeap(), 0, (keylen + 1) * sizeof(TCHAR));
  if (! key) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, _T("key"), _T("remove_from_double_null()"), 0);
    return 1;
  }
  ...
  *newdn = (TCHAR *) HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, len * sizeof(TCHAR));
  if (! *newdn) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, _T("newdn"), _T("remove_from_double_null()"), 0);
    HeapFree(GetProcessHeap(), 0, key);
    return 2;
  }
...
void override_milliseconds(TCHAR *service_name, HKEY key, TCHAR *value, unsigned long *buffer, unsigned long default_value, unsigned long event) {
  ...
  if (ret != ERROR_SUCCESS) {
    if (ret != ERROR_FILE_NOT_FOUND) {
      ...
      else log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_QUERYVALUE_FAILED, value, error_string(ret), 0);
    }
  }
  else ok = true;
...
HKEY open_service_registry(const TCHAR *service_name, REGSAM sam, bool must_exist) {
  ...
  if (service_registry_path(service_name, false, 0, registry, _countof(registry)) < 0) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, FCSM_REGISTRY, _T("open_service_registry()"), 0);
    return 0;
  }
...
long open_registry(const TCHAR *service_name, const TCHAR *sub, REGSAM sam, HKEY *key, bool must_exist) {
  ...
  if (service_registry_path(service_name, true, sub, registry, _countof(registry)) < 0) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, FCSM_REGISTRY, _T("open_registry()"), 0);
    return 0;
  }
...
int get_io_parameters(fcsm_service_t *service, HKEY key) {
  /* stdin */
  if (get_createfile_parameters(key, FCSM_REG_STDIN, service->stdin_path, &service->stdin_sharing, FCSM_STDIN_SHARING, &service->stdin_disposition, FCSM_STDIN_DISPOSITION, &service->stdin_flags, FCSM_STDIN_FLAGS, 0)) {
  ...
  /* stdout */
  if (get_createfile_parameters(key, FCSM_REG_STDOUT, service->stdout_path, &service->stdout_sharing, FCSM_STDOUT_SHARING, &service->stdout_disposition, FCSM_STDOUT_DISPOSITION, &service->stdout_flags, FCSM_STDOUT_FLAGS, &service->stdout_copy_and_truncate)) {
  ...
  /* stderr */
  if (get_createfile_parameters(key, FCSM_REG_STDERR, service->stderr_path, &service->stderr_sharing, FCSM_STDERR_SHARING, &service->stderr_disposition, FCSM_STDERR_DISPOSITION, &service->stderr_flags, FCSM_STDERR_FLAGS, &service->stderr_copy_and_truncate)) {
...
int get_parameters(fcsm_service_t *service, STARTUPINFO *si) {
  unsigned long ret;
  ...
  get_environment(service->name, key, FCSM_REG_ENV, &service->env, &service->envlen);
  /* Environment variables to add to existing rather than replace - may fail. */
  get_environment(service->name, key, FCSM_REG_ENV_EXTRA, &service->env_extra, &service->env_extralen);
  ...
  if (get_string(key, FCSM_REG_EXE, service->exe, sizeof(service->exe), expand, false, true)) {
    RegCloseKey(key);
    return 3;
  }

  /* Try to get flags - may fail and we don't care */
  if (get_string(key, FCSM_REG_FLAGS, service->flags, sizeof(service->flags), expand, false, true)) {
    log_event(EVENTLOG_WARNING_TYPE, FCSM_EVENT_NO_FLAGS, FCSM_REG_FLAGS, service->name, service->exe, 0);
    ZeroMemory(service->flags, sizeof(service->flags));
  }

  /* Try to get startup directory - may fail and we fall back to a default */
  if (get_string(key, FCSM_REG_DIR, service->dir, sizeof(service->dir), expand, true, true) || ! service->dir[0]) {
    ...
    if (service->dir[0] == _T('\0')) {
      ...
      if (! ret || ret > sizeof(service->dir)) {
        log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_NO_DIR_AND_NO_FALLBACK, FCSM_REG_DIR, service->name, 0);
        RegCloseKey(key);
        return 4;
      }
    }
    log_event(EVENTLOG_WARNING_TYPE, FCSM_EVENT_NO_DIR, FCSM_REG_DIR, service->name, service->dir, 0);
  }

  /* Try to get processor affinity - may fail. */
  TCHAR buffer[512];
  if (get_string(key, FCSM_REG_AFFINITY, buffer, sizeof(buffer), false, false, false) || ! buffer[0]) service->affinity = 0LL;
  else if (affinity_string_to_mask(buffer, &service->affinity)) {
    log_event(EVENTLOG_WARNING_TYPE, FCSM_EVENT_BOGUS_AFFINITY_MASK, service->name, buffer);
      ...
      if (effective_affinity != service->affinity) {
        ...
        if (! affinity_mask_to_string(system_affinity, &system)) {
          ...
          if (! affinity_mask_to_string(effective_affinity, &effective)) {
            log_event(EVENTLOG_WARNING_TYPE, FCSM_EVENT_EFFECTIVE_AFFINITY_MASK, service->name, buffer, system, effective, 0);
          }
  ...
  if (get_number(key, FCSM_REG_PRIORITY, &priority, false) == 1) {
    if (priority == (priority & priority_mask())) service->priority = priority;
    else log_event(EVENTLOG_WARNING_TYPE, FCSM_EVENT_BOGUS_PRIORITY, service->name, FCSM_REG_PRIORITY, 0);
  }

  /* Try to get hook I/O sharing - may fail. */
  unsigned long hook_share_output_handles;
  if (get_number(key, FCSM_REG_HOOK_SHARE_OUTPUT_HANDLES, &hook_share_output_handles, false) == 1) {
  ...
  if (get_number(key, FCSM_REG_ROTATE, &rotate_files, false) == 1) {
    if (rotate_files) service->rotate_files = true;
    else service->rotate_files = false;
  }
  else service->rotate_files = false;
  if (get_number(key, FCSM_REG_ROTATE_ONLINE, &rotate_files, false) == 1) {
  ...
  if (get_number(key, FCSM_REG_TIMESTAMP_LOG, &timestamp_log, false) == 1) {
  ...
  if (get_number(key, FCSM_REG_ROTATE_SECONDS, &service->rotate_seconds, false) != 1) service->rotate_seconds = 0;
  if (get_number(key, FCSM_REG_ROTATE_BYTES_LOW, &service->rotate_bytes_low, false) != 1) service->rotate_bytes_low = 0;
  if (get_number(key, FCSM_REG_ROTATE_BYTES_HIGH, &service->rotate_bytes_high, false) != 1) service->rotate_bytes_high = 0;
  override_milliseconds(service->name, key, FCSM_REG_ROTATE_DELAY, &service->rotate_delay, FCSM_ROTATE_DELAY, FCSM_EVENT_BOGUS_THROTTLE);

  /* Try to get force new console setting - may fail. */
  if (get_number(key, FCSM_REG_NO_CONSOLE, &service->no_console, false) != 1) service->no_console = 0;
  ...
  if (get_io_parameters(service, key)) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_GET_OUTPUT_HANDLES_FAILED, service->name, 0);
    RegCloseKey(key);
  ...
  override_milliseconds(service->name, key, FCSM_REG_RESTART_DELAY, &service->restart_delay, 0, FCSM_EVENT_BOGUS_RESTART_DELAY);

  /* Try to get throttle restart delay */
  override_milliseconds(service->name, key, FCSM_REG_THROTTLE, &service->throttle_delay, FCSM_RESET_THROTTLE_RESTART, FCSM_EVENT_BOGUS_THROTTLE);
  ...
  bool stop_ok = false;
  ret = RegQueryValueEx(key, FCSM_REG_STOP_METHOD_SKIP, 0, &type, (unsigned char *) &stop_method_skip, &buflen);
  if (ret != ERROR_SUCCESS) {
    if (ret != ERROR_FILE_NOT_FOUND) {
      if (type != REG_DWORD) {
        log_event(EVENTLOG_WARNING_TYPE, FCSM_EVENT_BOGUS_STOP_METHOD_SKIP, service->name, FCSM_REG_STOP_METHOD_SKIP, FCSM, 0);
      }
      else log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_QUERYVALUE_FAILED, FCSM_REG_STOP_METHOD_SKIP, error_string(ret), 0);
    }
  }
  else stop_ok = true;
  ...
  override_milliseconds(service->name, key, FCSM_REG_KILL_CONSOLE_GRACE_PERIOD, &service->kill_console_delay, FCSM_KILL_CONSOLE_GRACE_PERIOD, FCSM_EVENT_BOGUS_KILL_CONSOLE_GRACE_PERIOD);
  override_milliseconds(service->name, key, FCSM_REG_KILL_WINDOW_GRACE_PERIOD, &service->kill_window_delay, FCSM_KILL_WINDOW_GRACE_PERIOD, FCSM_EVENT_BOGUS_KILL_WINDOW_GRACE_PERIOD);
  override_milliseconds(service->name, key, FCSM_REG_KILL_THREADS_GRACE_PERIOD, &service->kill_threads_delay, FCSM_KILL_THREADS_GRACE_PERIOD, FCSM_EVENT_BOGUS_KILL_THREADS_GRACE_PERIOD);

  /* Try to get process tree settings - may fail. */
  unsigned long kill_process_tree;
  if (get_number(key, FCSM_REG_KILL_PROCESS_TREE, &kill_process_tree, false) == 1) {
  ...
  bool default_action;
  service->default_exit_action = FCSM_EXIT_RESTART;
...
int get_exit_action(const TCHAR *service_name, unsigned long *ret, TCHAR *action, bool *default_action) {
  ...
  *default_action = ! ret;
  ...
  /* Try to open the registry */
  HKEY key = open_registry(service_name, FCSM_REG_EXIT, KEY_READ);
  if (! key) return 1;
...
int set_hook(const TCHAR *service_name, const TCHAR *hook_event, const TCHAR *hook_action, TCHAR *cmd) {
  /* Try to open the registry */
  TCHAR registry[KEY_LENGTH];
  if (_sntprintf_s(registry, _countof(registry), _TRUNCATE, _T("%s\\%s"), FCSM_REG_HOOK, hook_event) < 0) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, _T("hook registry"), _T("set_hook()"), 0);
    return 1;
  }
...
int get_hook(const TCHAR *service_name, const TCHAR *hook_event, const TCHAR *hook_action, TCHAR *buffer, unsigned long buflen) {
  ...
  TCHAR registry[KEY_LENGTH];
  if (_sntprintf_s(registry, _countof(registry), _TRUNCATE, _T("%s\\%s"), FCSM_REG_HOOK, hook_event) < 0) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, _T("hook registry"), _T("get_hook()"), 0);
    return 1;
  }
  HKEY key;
