...
void service_kill_t(fcsm_service_t *service, kill_t *k) {
  if (! service) return;
  if (! k) return;
...
int get_process_creation_time(HANDLE process_handle, FILETIME *ft) {
  ...
  if (! GetProcessTimes(process_handle, &creation_time, &exit_time, &kernel_time, &user_time)) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_GETPROCESSTIMES_FAILED, error_string(GetLastError()), 0);
    return 1;
  }
...
int check_parent(kill_t *k, PROCESSENTRY32 *pe, unsigned long ppid) {
  ...
  */
  ...
  if (! process_handle) {
    ...
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OPENPROCESS_FAILED, pid_string, k->name, error_string(GetLastError()), 0);
    return 2;
  }
...
*/
int kill_threads(fcsm_service_t *service, kill_t *k) {
  int ret = 0;

  /* Get a snapshot of all threads in the system. */
  HANDLE snapshot = CreateToolhelp32Snapshot(TH32CS_SNAPTHREAD, 0);
  if (snapshot == INVALID_HANDLE_VALUE) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_CREATETOOLHELP32SNAPSHOT_THREAD_FAILED, k->name, error_string(GetLastError()), 0);
    return 0;
  }
  ...
  if (! Thread32First(snapshot, &te)) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_THREAD_ENUMERATE_FAILED, k->name, error_string(GetLastError()), 0);
...
int kill_process(fcsm_service_t *service, kill_t *k) {
  if (! k) return 1;
  ...
  if (k->stop_method & FCSM_STOP_METHOD_CONSOLE) {
  ...
  */
  if (k->stop_method & FCSM_STOP_METHOD_WINDOW) {
  ...
  */
  if (k->stop_method & FCSM_STOP_METHOD_THREADS) {
    if (kill_threads(k)) {
  ...
  if (k->stop_method & FCSM_STOP_METHOD_TERMINATE) {
...
int kill_console(fcsm_service_t *service, kill_t *k) {
  unsigned long ret;
  ...
  if (! imports.AttachConsole(k->pid)) {
    ...
    switch (ret) {
      ...
      default:
        ...
        log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_ATTACHCONSOLE_FAILED, k->name, error_string(ret), 0);
        return 3;
    }
  }
  ...
  if (! ignored) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_SETCONSOLECTRLHANDLER_FAILED, k->name, error_string(GetLastError()), 0);
    ret = 4;
  }
  ...
  if (! ret) {
    if (! GenerateConsoleCtrlEvent(CTRL_C_EVENT, 0)) {
      log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_GENERATECONSOLECTRLEVENT_FAILED, k->name, error_string(GetLastError()), 0);
      ret = 5;
    }
  }
  ...
  if (! FreeConsole()) {
    log_event(EVENTLOG_WARNING_TYPE, FCSM_EVENT_FREECONSOLE_FAILED, k->name, error_string(GetLastError()), 0);
  }
...
void walk_process_tree(fcsm_service_t *service, walk_function_t fn, kill_t *k, unsigned long ppid) {
  if (! k) return;
  ...
  if (fn == kill_process) log_event(EVENTLOG_INFORMATION_TYPE, FCSM_EVENT_KILLING, k->name, pid_string, code, 0);
  ...
  if (process_handle) {
    ...
    if (fn == kill_process) log_event(EVENTLOG_INFORMATION_TYPE, FCSM_EVENT_KILL_PROCESS_TREE, pid_string, ppid_string, k->name, 0);
    ...
    if (! fn(service, k)) {
      ...
      if (! GetExitCodeProcess(process_handle, &ret) || ret == STILL_ACTIVE) {
        if (k->stop_method & FCSM_STOP_METHOD_TERMINATE) log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_TERMINATEPROCESS_FAILED, pid_string, k->name, error_string(GetLastError()), 0);
        else log_event(EVENTLOG_WARNING_TYPE, FCSM_EVENT_PROCESS_STILL_ACTIVE, k->name, pid_string, FCSM, FCSM_REG_STOP_METHOD_SKIP, 0);
      }
    }

    CloseHandle(process_handle);
  }
  else log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OPENPROCESS_FAILED, pid_string, k->name, error_string(GetLastError()), 0);

  /* Get a snapshot of all processes in the system. */
  HANDLE snapshot = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
  if (snapshot == INVALID_HANDLE_VALUE) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_CREATETOOLHELP32SNAPSHOT_PROCESS_FAILED, k->name, error_string(GetLastError()), 0);
    return;
  }
  ...
  if (! Process32First(snapshot, &pe)) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_PROCESS_ENUMERATE_FAILED, k->name, error_string(GetLastError()), 0);
    CloseHandle(snapshot);
    return;
  }
...
int print_process(fcsm_service_t *service, kill_t *k) {
