...
/* Affinity. */
#define FCSM_AFFINITY_ALL _T("All")
/* Default value. */
#define FCSM_DEFAULT_STRING _T("Default")
...
static inline int is_default(const TCHAR *value) {
  return (str_equiv(value, FCSM_DEFAULT_STRING) || str_equiv(value, _T("*")) || ! value[0]);
}
...
static int value_from_string(const TCHAR *name, value_t *value, const TCHAR *string) {
  ...
  if (! value->string) {
    print_message(stderr, FCSM_MESSAGE_OUT_OF_MEMORY, name, _T("value_from_string()"));
    return -1;
  }
...
/* Functions to manage FCSM-specific settings in the registry. */
static int setting_set_number(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  if (! value || ! value->string) {
    ...
    print_message(stderr, FCSM_MESSAGE_REGDELETEVALUE_FAILED, name, service_name, error_string(error));
    return -1;
  }
...
static int setting_dump_string(const TCHAR *service_name, void *param, const TCHAR *name, const value_t *value, const TCHAR *additional) {
  ...
  if (quote(fcsm_exe(), quoted_fcsm, _countof(quoted_fcsm))) return 3;
  if (_tcslen(quoted_additional)) _tprintf(_T("%s set %s %s %s %s\n"), quoted_fcsm, quoted_service_name, name, quoted_additional, quoted_value);
  else _tprintf(_T("%s set %s %s %s\n"), quoted_fcsm, quoted_service_name, name, quoted_value);
  return 0;
}
...
static int setting_set_exit_action(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  if (value && value->string) _sntprintf_s(action_string, _countof(action_string), _TRUNCATE, _T("%s"), value->string);
    ...
    if (code) {
      ...
      RegCloseKey(key);
      if (error == ERROR_SUCCESS || error == ERROR_FILE_NOT_FOUND) return 0;
      print_message(stderr, FCSM_MESSAGE_REGDELETEVALUE_FAILED, code, service_name, error_string(error));
      return -1;
    }
    else {
  ...
  for (int i = 0; exit_action_strings[i]; i++) {
    if (! _tcsnicmp((const TCHAR *) action_string, exit_action_strings[i], ACTION_LEN)) {
      ...
      if (RegSetValueEx(key, code, 0, REG_SZ, (const unsigned char *) exit_action_strings[i], (unsigned long) (_tcslen(action_string) + 1) * sizeof(TCHAR)) != ERROR_SUCCESS) {
        print_message(stderr, FCSM_MESSAGE_SETVALUE_FAILED, code, service_name, error_string(GetLastError()));
        RegCloseKey(key);
        return -1;
      }
  ...
  print_message(stderr, FCSM_MESSAGE_INVALID_EXIT_ACTION, action_string);
...
static int setting_dump_exit_action(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  while (true) {
    ...
    TCHAR *additional = (code[i]) ? code : FCSM_DEFAULT_STRING;
...
static inline bool split_hook_name(const TCHAR *hook_name, TCHAR *hook_event, TCHAR *hook_action) {
  ...
  print_message(stderr, FCSM_MESSAGE_INVALID_HOOK_NAME, hook_name);
  return false;
}
...
static int setting_set_affinity(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  if (value && value->string) {
    ...
    if (is_default(value->string) || str_equiv(value->string, FCSM_AFFINITY_ALL)) mask = 0LL;
    else if (affinity_string_to_mask(value->string, &mask)) {
      print_message(stderr, FCSM_MESSAGE_BOGUS_AFFINITY_MASK, value->string, num_cpus() - 1);
      return -1;
    }
  }
  else mask = 0LL;
  ...
  if (effective_affinity != mask) {
    ...
    if (! affinity_mask_to_string(system_affinity, &system)) {
      ...
      if (! affinity_mask_to_string(effective_affinity, &effective)) {
        print_message(stderr, FCSM_MESSAGE_EFFECTIVE_AFFINITY_MASK, value->string, system, effective);
  ...
  if (RegSetValueEx(key, name, 0, REG_SZ, (const unsigned char *) canon, (unsigned long) (_tcslen(canon) + 1) * sizeof(TCHAR)) != ERROR_SUCCESS) {
    ...
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_SETVALUE_FAILED, name, error_string(GetLastError()), 0);
    return -1;
  }
...
static int setting_get_affinity(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  if (ret == ERROR_FILE_NOT_FOUND) {
    if (value_from_string(name, value, FCSM_AFFINITY_ALL) == 1) return 0;
    return -1;
  }
  ...
  if (! buffer) {
    print_message(stderr, FCSM_MESSAGE_OUT_OF_MEMORY, _T("affinity"), _T("setting_get_affinity"));
    return -1;
  }
  ...
  if (affinity_string_to_mask(buffer, &affinity)) {
    print_message(stderr, FCSM_MESSAGE_BOGUS_AFFINITY_MASK, buffer, num_cpus() - 1);
    HeapFree(GetProcessHeap(), 0, buffer);
    return -1;
  }
...
static int setting_set_environment(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  if (test_environment(unformatted)) {
    ...
    print_message(stderr, FCSM_GUI_INVALID_ENVIRONMENT);
    return -1;
  }
...
static int setting_dump_environment(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  for (s = env; *s; s++) {
    ...
    value->string = (TCHAR *) HeapAlloc(GetProcessHeap(), 0, len * sizeof(TCHAR));
    if (! value->string) {
      print_message(stderr, FCSM_MESSAGE_OUT_OF_MEMORY, _T("dump"), _T("setting_dump_environment"));
      break;
    }
...
static int setting_set_priority(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  print_message(stderr, FCSM_MESSAGE_INVALID_PRIORITY, priority_string);
  ...
  if (! op) {
    ...
    if (type == DEPENDENCY_GROUPS) {
      ...
      if (missing) {
        ...
        if (! canon) {
          print_message(stderr, FCSM_MESSAGE_OUT_OF_MEMORY, _T("canon"), _T("native_set_dependon"));
...
static int native_set_dependongroup(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  */
  ...
  if (services_buflen > 2) {
    ...
    if (! dependencies) {
      print_message(stderr, FCSM_MESSAGE_OUT_OF_MEMORY, _T("dependencies"), _T("native_set_dependongroup"));
...
static int setting_dump_dependon(const TCHAR *service_name, SC_HANDLE service_handle, const TCHAR *name, int type, value_t *value) {
  ...
  for (s = dependencies; *s; s++) {
    ...
    value->string = (TCHAR *) HeapAlloc(GetProcessHeap(), 0, len * sizeof(TCHAR));
    if (! value->string) {
      print_message(stderr, FCSM_MESSAGE_OUT_OF_MEMORY, _T("dump"), _T("setting_dump_dependon"));
      break;
    }
...
static int native_set_dependonservice(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  */
  ...
  if (groups_buflen > 2) {
    ...
    if (! dependencies) {
      print_message(stderr, FCSM_MESSAGE_OUT_OF_MEMORY, _T("dependencies"), _T("native_set_dependonservice"));
      if (groups_buffer) HeapFree(GetProcessHeap(), 0, groups_buffer);
      if (services_buffer) HeapFree(GetProcessHeap(), 0, services_buffer);
      return -1;
    }
...
int native_set_imagepath(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  if (! value || ! value->string) {
    print_message(stderr, FCSM_MESSAGE_NO_DEFAULT_VALUE, name);
    return -1;
  }
...
int native_set_name(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  print_message(stderr, FCSM_MESSAGE_CANNOT_RENAME_SERVICE);
  return -1;
}
...
int native_set_objectname(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  */
  ...
  if (well_known) {
    if (str_equiv(well_known, FCSM_LOCALSYSTEM_ACCOUNT)) localsystem = true;
    ...
    print_message(stderr, FCSM_MESSAGE_MISSING_PASSWORD, name);
    return -1;
  }
...
int native_dump_objectname(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  if (! _tcsnicmp(FCSM_VIRTUAL_SERVICE_ACCOUNT_DOMAIN, value->string, _tcslen(FCSM_VIRTUAL_SERVICE_ACCOUNT_DOMAIN))) {
...
int native_set_startup(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  /* Map FCSM_STARTUP_* constant to Windows SERVICE_*_START constant. */
  ...
  if (service_startup < 0) {
    print_message(stderr, FCSM_MESSAGE_INVALID_SERVICE_STARTUP, value->string);
  ...
  if (service_startup == FCSM_STARTUP_DELAYED) delayed.fDelayedAutostart = 1;
  ...
  if (! ChangeServiceConfig2(service_handle, SERVICE_CONFIG_DELAYED_AUTO_START_INFO, &delayed)) {
    ...
    if (error != ERROR_INVALID_LEVEL) {
      log_event(EVENTLOG_ERROR_TYPE, FCSM_MESSAGE_SERVICE_CONFIG_DELAYED_AUTO_START_INFO_FAILED, service_name, error_string(error), 0);
    }
  }
...
int native_set_type(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  */
  ...
  if (str_equiv(value->string, FCSM_INTERACTIVE_PROCESS)) type |= SERVICE_INTERACTIVE_PROCESS;
  else if (! str_equiv(value->string, FCSM_WIN32_OWN_PROCESS)) {
    print_message(stderr, FCSM_MESSAGE_INVALID_SERVICE_TYPE, value->string);
    _ftprintf(stderr, _T("%s\n"), FCSM_WIN32_OWN_PROCESS);
    _ftprintf(stderr, _T("%s\n"), FCSM_INTERACTIVE_PROCESS);
    return -1;
  }
  ...
  */
  if (type & SERVICE_INTERACTIVE_PROCESS) {
    ...
    if (! str_equiv(qsc->lpServiceStartName, FCSM_LOCALSYSTEM_ACCOUNT)) {
      HeapFree(GetProcessHeap(), 0, qsc);
      print_message(stderr, FCSM_MESSAGE_INTERACTIVE_NOT_LOCALSYSTEM, value->string, service_name, FCSM_LOCALSYSTEM_ACCOUNT);
      return -1;
    }
...
int native_get_type(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  switch (value->numeric) {
    case SERVICE_KERNEL_DRIVER: string = FCSM_KERNEL_DRIVER; break;
    case SERVICE_FILE_SYSTEM_DRIVER: string = FCSM_FILE_SYSTEM_DRIVER; break;
    case SERVICE_WIN32_OWN_PROCESS: string = FCSM_WIN32_OWN_PROCESS; break;
    case SERVICE_WIN32_SHARE_PROCESS: string = FCSM_WIN32_SHARE_PROCESS; break;
    case SERVICE_WIN32_OWN_PROCESS|SERVICE_INTERACTIVE_PROCESS: string = FCSM_INTERACTIVE_PROCESS; break;
    case SERVICE_WIN32_SHARE_PROCESS|SERVICE_INTERACTIVE_PROCESS: string = FCSM_SHARE_INTERACTIVE_PROCESS; break;
    default: string = FCSM_UNKNOWN;
  }
...
int set_setting(const TCHAR *service_name, HKEY key, settings_t *setting, value_t *value, const TCHAR *additional) {
  ...
  if (! ret) print_message(stdout, FCSM_MESSAGE_RESET_SETTING, setting->name, service_name);
  else if (ret > 0) print_message(stdout, FCSM_MESSAGE_SET_SETTING, setting->name, service_name);
  else print_message(stderr, FCSM_MESSAGE_SET_SETTING_FAILED, setting->name, service_name);
...
int get_setting(const TCHAR *service_name, HKEY key, settings_t *setting, value_t *value, const TCHAR *additional) {
  ...
  if (ret < 0) print_message(stderr, FCSM_MESSAGE_GET_SETTING_FAILED, setting->name, service_name);
...
settings_t settings[] = {
  { FCSM_REG_EXE, REG_EXPAND_SZ, (void *) _T(""), false, 0, setting_set_string, setting_get_string, setting_not_dumpable },
  { FCSM_REG_FLAGS, REG_EXPAND_SZ, (void *) _T(""), false, 0, setting_set_string, setting_get_string, 0 },
  { FCSM_REG_DIR, REG_EXPAND_SZ, (void *) _T(""), false, 0, setting_set_string, setting_get_string, 0 },
  { FCSM_REG_EXIT, REG_SZ, (void *) exit_action_strings[FCSM_EXIT_RESTART], false, ADDITIONAL_MANDATORY, setting_set_exit_action, setting_get_exit_action, setting_dump_exit_action },
  { FCSM_REG_HOOK, REG_SZ, (void *) _T(""), false, ADDITIONAL_MANDATORY, setting_set_hook, setting_get_hook, setting_dump_hooks },
  { FCSM_REG_AFFINITY, REG_SZ, 0, false, 0, setting_set_affinity, setting_get_affinity, 0 },
  { FCSM_REG_ENV, REG_MULTI_SZ, NULL, false, ADDITIONAL_CRLF, setting_set_environment, setting_get_environment, setting_dump_environment },
  { FCSM_REG_ENV_EXTRA, REG_MULTI_SZ, NULL, false, ADDITIONAL_CRLF, setting_set_environment, setting_get_environment, setting_dump_environment },
  { FCSM_REG_NO_CONSOLE, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_PRIORITY, REG_SZ, (void *) priority_strings[FCSM_NORMAL_PRIORITY], false, 0, setting_set_priority, setting_get_priority, setting_dump_priority },
  { FCSM_REG_RESTART_DELAY, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_STDIN, REG_EXPAND_SZ, NULL, false, 0, setting_set_string, setting_get_string, 0 },
  { FCSM_REG_STDIN FCSM_REG_STDIO_SHARING, REG_DWORD, (void *) FCSM_STDIN_SHARING, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_STDIN FCSM_REG_STDIO_DISPOSITION, REG_DWORD, (void *) FCSM_STDIN_DISPOSITION, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_STDIN FCSM_REG_STDIO_FLAGS, REG_DWORD, (void *) FCSM_STDIN_FLAGS, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_STDOUT, REG_EXPAND_SZ, NULL, false, 0, setting_set_string, setting_get_string, 0 },
  { FCSM_REG_STDOUT FCSM_REG_STDIO_SHARING, REG_DWORD, (void *) FCSM_STDOUT_SHARING, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_STDOUT FCSM_REG_STDIO_DISPOSITION, REG_DWORD, (void *) FCSM_STDOUT_DISPOSITION, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_STDOUT FCSM_REG_STDIO_FLAGS, REG_DWORD, (void *) FCSM_STDOUT_FLAGS, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_STDOUT FCSM_REG_STDIO_COPY_AND_TRUNCATE, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_STDERR, REG_EXPAND_SZ, NULL, false, 0, setting_set_string, setting_get_string, 0 },
  { FCSM_REG_STDERR FCSM_REG_STDIO_SHARING, REG_DWORD, (void *) FCSM_STDERR_SHARING, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_STDERR FCSM_REG_STDIO_DISPOSITION, REG_DWORD, (void *) FCSM_STDERR_DISPOSITION, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_STDERR FCSM_REG_STDIO_FLAGS, REG_DWORD, (void *) FCSM_STDERR_FLAGS, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_STDERR FCSM_REG_STDIO_COPY_AND_TRUNCATE, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_STOP_METHOD_SKIP, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_KILL_CONSOLE_GRACE_PERIOD, REG_DWORD, (void *) FCSM_KILL_CONSOLE_GRACE_PERIOD, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_KILL_WINDOW_GRACE_PERIOD, REG_DWORD, (void *) FCSM_KILL_WINDOW_GRACE_PERIOD, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_KILL_THREADS_GRACE_PERIOD, REG_DWORD, (void *) FCSM_KILL_THREADS_GRACE_PERIOD, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_KILL_PROCESS_TREE, REG_DWORD, (void *) 1, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_THROTTLE, REG_DWORD, (void *) FCSM_RESET_THROTTLE_RESTART, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_HOOK_SHARE_OUTPUT_HANDLES, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_ROTATE, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_ROTATE_ONLINE, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_ROTATE_SECONDS, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_ROTATE_BYTES_LOW, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_ROTATE_BYTES_HIGH, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_ROTATE_DELAY, REG_DWORD, (void *) FCSM_ROTATE_DELAY, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_REG_TIMESTAMP_LOG, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { FCSM_NATIVE_DEPENDONGROUP, REG_MULTI_SZ, NULL, true, ADDITIONAL_CRLF, native_set_dependongroup, native_get_dependongroup, native_dump_dependongroup },
  { FCSM_NATIVE_DEPENDONSERVICE, REG_MULTI_SZ, NULL, true, ADDITIONAL_CRLF, native_set_dependonservice, native_get_dependonservice, native_dump_dependonservice },
  { FCSM_NATIVE_DESCRIPTION, REG_SZ, _T(""), true, 0, native_set_description, native_get_description, 0 },
  { FCSM_NATIVE_DISPLAYNAME, REG_SZ, NULL, true, 0, native_set_displayname, native_get_displayname, 0 },
  { FCSM_NATIVE_ENVIRONMENT, REG_MULTI_SZ, NULL, true, ADDITIONAL_CRLF, native_set_environment, native_get_environment, native_dump_environment },
  { FCSM_NATIVE_IMAGEPATH, REG_EXPAND_SZ, NULL, true, 0, native_set_imagepath, native_get_imagepath, setting_not_dumpable },
  { FCSM_NATIVE_OBJECTNAME, REG_SZ, FCSM_LOCALSYSTEM_ACCOUNT, true, 0, native_set_objectname, native_get_objectname, native_dump_objectname },
  { FCSM_NATIVE_NAME, REG_SZ, NULL, true, 0, native_set_name, native_get_name, setting_not_dumpable },
  { FCSM_NATIVE_STARTUP, REG_SZ, NULL, true, 0, native_set_startup, native_get_startup, 0 },
  { FCSM_NATIVE_TYPE, REG_SZ, NULL, true, 0, native_set_type, native_get_type, 0 },
