...
TCHAR *expand_environment_string(TCHAR *string) {
  ...
  if (! len) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_EXPANDENVIRONMENTSTRINGS_FAILED, string, error_string(GetLastError()), 0);
    return 0;
  }

  TCHAR *ret = (TCHAR *) HeapAlloc(GetProcessHeap(), 0, len * sizeof(TCHAR));
  if (! ret) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, _T("ExpandEnvironmentStrings()"), _T("expand_environment_string"), 0);
    return 0;
  }
...
int test_environment(TCHAR *env) {
  TCHAR *path = (TCHAR *) fcsm_imagepath();
  STARTUPINFO si;
...
int remove_from_environment_block(TCHAR *env, unsigned long envlen, TCHAR *string, TCHAR **newenv, unsigned long *newlen) {
  ...
  if (! key) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, _T("key"), _T("remove_from_environment_block()"), 0);
    return 2;
  }
