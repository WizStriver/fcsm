...
static int dup_handle(HANDLE source_handle, HANDLE *dest_handle_ptr, TCHAR *source_description, TCHAR *dest_description, unsigned long flags) {
  ...
  if (! DuplicateHandle(GetCurrentProcess(), source_handle, GetCurrentProcess(), dest_handle_ptr, 0, true, flags)) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_DUPLICATEHANDLE_FAILED, source_description, dest_description, error_string(GetLastError()), 0);
    return 2;
  }
  return 0;
}
...
static HANDLE create_logging_thread(TCHAR *service_name, TCHAR *path, unsigned long sharing, unsigned long disposition, unsigned long flags, HANDLE *read_handle_ptr, HANDLE *pipe_handle_ptr, HANDLE *write_handle_ptr, unsigned long rotate_bytes_low, unsigned long rotate_bytes_high, unsigned long rotate_delay, unsigned long *tid_ptr, unsigned long *rotate_online, bool timestamp_log, bool copy_and_truncate) {
  *tid_ptr = 0;
  ...
  if (read_handle_ptr && ! *read_handle_ptr) {
    if (pipe_handle_ptr && ! *pipe_handle_ptr) {
      if (CreatePipe(read_handle_ptr, pipe_handle_ptr, 0, 0)) {
        ...
        log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_CREATEPIPE_FAILED, service_name, path, error_string(GetLastError()));
        return (HANDLE) 0;
      }
    }
  }
  ...
  if (! logger) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, _T("logger"), _T("create_logging_thread()"), 0);
    return (HANDLE) 0;
  }
...
static inline void write_bom(logger_t *logger, unsigned long *out) {
  ...
  if (! WriteFile(logger->write_handle, (void *) &bom, sizeof(bom), out, 0)) {
    log_event(EVENTLOG_WARNING_TYPE, FCSM_EVENT_SOMEBODY_SET_UP_US_THE_BOM, logger->service_name, logger->path, error_string(GetLastError()), 0);
  }
}
...
int get_createfile_parameters(HKEY key, TCHAR *prefix, TCHAR *path, unsigned long *sharing, unsigned long default_sharing, unsigned long *disposition, unsigned long default_disposition, unsigned long *flags, unsigned long default_flags, bool *copy_and_truncate) {
  TCHAR value[FCSM_STDIO_LENGTH];

  /* Path. */
  if (_sntprintf_s(value, _countof(value), _TRUNCATE, _T("%s"), prefix) < 0) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, prefix, _T("get_createfile_parameters()"), 0);
    return 1;
  }
  ...
  /* ShareMode. */
  if (_sntprintf_s(value, _countof(value), _TRUNCATE, _T("%s%s"), prefix, FCSM_REG_STDIO_SHARING) < 0) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, FCSM_REG_STDIO_SHARING, _T("get_createfile_parameters()"), 0);
    return 3;
  }
  ...
  /* CreationDisposition. */
  if (_sntprintf_s(value, _countof(value), _TRUNCATE, _T("%s%s"), prefix, FCSM_REG_STDIO_DISPOSITION) < 0) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, FCSM_REG_STDIO_DISPOSITION, _T("get_createfile_parameters()"), 0);
    return 5;
  }
  ...
  /* Flags. */
  if (_sntprintf_s(value, _countof(value), _TRUNCATE, _T("%s%s"), prefix, FCSM_REG_STDIO_FLAGS) < 0) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, FCSM_REG_STDIO_FLAGS, _T("get_createfile_parameters()"), 0);
    return 7;
  }
  ...
  if (copy_and_truncate) {
    unsigned long data;
    if (_sntprintf_s(value, _countof(value), _TRUNCATE, _T("%s%s"), prefix, FCSM_REG_STDIO_COPY_AND_TRUNCATE) < 0) {
      log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, FCSM_REG_STDIO_COPY_AND_TRUNCATE, _T("get_createfile_parameters()"), 0);
      return 9;
    }
...
int set_createfile_parameter(HKEY key, TCHAR *prefix, TCHAR *suffix, unsigned long number) {
  ...
  if (_sntprintf_s(value, _countof(value), _TRUNCATE, _T("%s%s"), prefix, suffix) < 0) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, suffix, _T("set_createfile_parameter()"), 0);
    return 1;
  }
...
int delete_createfile_parameter(HKEY key, TCHAR *prefix, TCHAR *suffix) {
  ...
  if (_sntprintf_s(value, _countof(value), _TRUNCATE, _T("%s%s"), prefix, suffix) < 0) {
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, suffix, _T("delete_createfile_parameter()"), 0);
    return 1;
  }
...
HANDLE write_to_file(TCHAR *path, unsigned long sharing, SECURITY_ATTRIBUTES *attributes, unsigned long disposition, unsigned long flags) {
  ...
  log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_CREATEFILE_FAILED, path, error_string(GetLastError()), 0);
  return ret;
}
...
void rotate_file(TCHAR *service_name, TCHAR *path, unsigned long seconds, unsigned long delay, unsigned long low, unsigned long high, bool copy_and_truncate) {
  ...
  if (file != INVALID_HANDLE_VALUE) {
    ...
    log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_ROTATE_FILE_FAILED, service_name, path, _T("CreateFile()"), path, error_string(error), 0);
    /* Reuse current time for rotation timestamp. */
    seconds = low = high = 0;
    SystemTimeToFileTime(&st, &info.ftLastWriteTime);
  }
  ...
  if (copy_and_truncate) {
    ...
    if (CopyFile(path, rotated, TRUE)) {
      file = write_to_file(path, FCSM_STDOUT_SHARING, 0, FCSM_STDOUT_DISPOSITION, FCSM_STDOUT_FLAGS);
      Sleep(delay);
  ...
  if (ok) {
    log_event(EVENTLOG_INFORMATION_TYPE, FCSM_EVENT_ROTATED, service_name, path, rotated, 0);
    return;
  }
  error = GetLastError();

  if (error == ERROR_FILE_NOT_FOUND) return;
  log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_ROTATE_FILE_FAILED, service_name, path, function, rotated, error_string(error), 0);
  return;
}

int get_output_handles(fcsm_service_t *service, STARTUPINFO *si) {
  if (! si) return 1;
  ...
  if (service->stdin_path[0]) {
    ...
    if (si->hStdInput == INVALID_HANDLE_VALUE) {
      log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_CREATEFILE_FAILED, service->stdin_path, error_string(GetLastError()), 0);
      return 2;
    }
  ...
  if (service->stdout_path[0]) {
    ...
    if (! service->stdout_thread) {
      if (dup_handle(stdout_handle, &service->stdout_si, FCSM_REG_STDOUT, _T("stdout"), DUPLICATE_CLOSE_SOURCE | DUPLICATE_SAME_ACCESS)) return 4;
      service->rotate_stdout_online = FCSM_ROTATE_OFFLINE;
    }
  ...
  if (service->stderr_path[0]) {
    ...
    if (str_equiv(service->stderr_path, service->stdout_path)) {
      ...
      service->rotate_stderr_online = FCSM_ROTATE_OFFLINE;
      ...
      if (! service->stderr_thread) {
        if (dup_handle(stderr_handle, &service->stderr_si, FCSM_REG_STDERR, _T("stderr"), DUPLICATE_CLOSE_SOURCE | DUPLICATE_SAME_ACCESS)) return 7;
        service->rotate_stderr_online = FCSM_ROTATE_OFFLINE;
      }
    }
...
int use_output_handles(fcsm_service_t *service, STARTUPINFO *si) {
...
void cleanup_loggers(fcsm_service_t *service) {
  unsigned long interval = FCSM_CLEANUP_LOGGERS_DEADLINE;
...
complain_read:
  ...
  if (*logger->rotate_online != FCSM_ROTATE_ONLINE) return ret;
  if (! (*complained & COMPLAINED_READ)) log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_READFILE_FAILED, logger->service_name, logger->path, error_string(error), 0);
...
complain_write:
  if (! (*complained & COMPLAINED_WRITE)) log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_WRITEFILE_FAILED, logger->service_name, logger->path, error_string(error), 0);
...
unsigned long WINAPI log_and_rotate(void *arg) {
  ...
  while (true) {
    ...
    if (*logger->rotate_online == FCSM_ROTATE_ONLINE_ASAP || (logger->size && size + (__int64) in >= logger->size)) {
      /* Look for newline. */
      unsigned long i;
      for (i = 0; i < in; i++) {
        if (buffer[i] == '\n') {
          ...
          /* Rotate. */
          *logger->rotate_online = FCSM_ROTATE_ONLINE;
          TCHAR rotated[PATH_LENGTH];
          ...
          */
          ...
          if (logger->copy_and_truncate) {
            ...
            if (CopyFile(logger->path, rotated, TRUE)) {
              HANDLE file = write_to_file(logger->path, FCSM_STDOUT_SHARING, 0, FCSM_STDOUT_DISPOSITION, FCSM_STDOUT_FLAGS);
          ...
          if (ok) {
            log_event(EVENTLOG_INFORMATION_TYPE, FCSM_EVENT_ROTATED, logger->service_name, logger->path, rotated, 0);
            size = 0LL;
          }
          else {
            error = GetLastError();
            if (error != ERROR_FILE_NOT_FOUND) {
              if (! (complained & COMPLAINED_ROTATE)) log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_ROTATE_FILE_FAILED, logger->service_name, logger->path, function, rotated, error_string(error), 0);
          ...
          if (logger->write_handle == INVALID_HANDLE_VALUE) {
            error = GetLastError();
            log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_CREATEFILE_FAILED, logger->path, error_string(error), 0);
