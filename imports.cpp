...
HMODULE get_dll(const TCHAR *dll, unsigned long *error) {
  *error = 0;
  ...
  if (! ret) {
    *error = GetLastError();
    if (*error != ERROR_PROC_NOT_FOUND) log_event(EVENTLOG_WARNING_TYPE, FCSM_EVENT_LOADLIBRARY_FAILED, dll, error_string(*error), 0);
  }
    ...
    *error = GetLastError();
    if (*error != ERROR_PROC_NOT_FOUND) {
      ...
      if (! from_utf8(function, &function_name, 0)) {
        log_event(EVENTLOG_WARNING_TYPE, FCSM_EVENT_GETPROCADDRESS_FAILED, function_name, error_string(*error), 0);
