...
int pre_edit_service(int argc, TCHAR** argv) {
	...
	if (!service->exe[0]) {
		service->native = true;
		if (mode != MODE_GETTING && mode != MODE_DUMPING) print_message(stderr, NSSM_MESSAGE_INVALID_SERVICE, service->name, FCSM, service->image);
	}
	...
	if (!setting->native && service->native) {
		CloseServiceHandle(service->handle);
		print_message(stderr, NSSM_MESSAGE_NATIVE_PARAMETER, setting->name, FCSM);
		return 1;
	}
