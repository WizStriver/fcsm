...
static inline int service_control_response(unsigned long control, unsigned long status) {
	switch (control) {
	case FCSM_SERVICE_CONTROL_START:
		switch (status) {
	...
	case FCSM_SERVICE_CONTROL_ROTATE:
		return 0;
	}
...
static inline void wait_for_hooks(fcsm_service_t* service, bool notify) {
	...
	await_hook_threads(&hook_threads, status_handle, status, FCSM_HOOK_THREAD_DEADLINE);
...
int priority_constant_to_index(unsigned long constant) {
	...
	case REALTIME_PRIORITY_CLASS: return FCSM_REALTIME_PRIORITY;
	case HIGH_PRIORITY_CLASS: return FCSM_HIGH_PRIORITY;
	case ABOVE_NORMAL_PRIORITY_CLASS: return FCSM_ABOVE_NORMAL_PRIORITY;
	case BELOW_NORMAL_PRIORITY_CLASS: return FCSM_BELOW_NORMAL_PRIORITY;
	case IDLE_PRIORITY_CLASS: return FCSM_IDLE_PRIORITY;
	}
	return FCSM_NORMAL_PRIORITY;
}

unsigned long priority_index_to_constant(int index) {
	switch (index) {
	case FCSM_REALTIME_PRIORITY: return REALTIME_PRIORITY_CLASS;
	case FCSM_HIGH_PRIORITY: return HIGH_PRIORITY_CLASS;
	case FCSM_ABOVE_NORMAL_PRIORITY: return ABOVE_NORMAL_PRIORITY_CLASS;
	case FCSM_BELOW_NORMAL_PRIORITY: return BELOW_NORMAL_PRIORITY_CLASS;
	case FCSM_IDLE_PRIORITY: return IDLE_PRIORITY_CLASS;
	}
...
void set_service_environment(fcsm_service_t* service) {
	if (!service) return;
...
void unset_service_environment(fcsm_service_t* service) {
	if (!service) return;
...
static unsigned long WINAPI shutdown_service(void* arg) {
	return stop_service((fcsm_service_t*)arg, 0, true, true);
}
...
static unsigned long WINAPI launch_service(void* arg) {
	return monitor_service((fcsm_service_t*)arg);
}
...
SC_HANDLE open_service_manager(unsigned long access) {
	...
	if (!ret) {
		if (is_admin) log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OPENSCMANAGER_FAILED, 0);
		return 0;
	}
...
SC_HANDLE open_service(SC_HANDLE services, TCHAR* service_name, unsigned long access, TCHAR* canonical_name, unsigned long canonical_namelen) {
	...
	if (error != ERROR_SERVICE_DOES_NOT_EXIST) {
		print_message(stderr, FCSM_MESSAGE_OPENSERVICE_FAILED, error_string(GetLastError()));
		return 0;
	}
	...
	if (error != ERROR_MORE_DATA) {
		print_message(stderr, FCSM_MESSAGE_ENUMSERVICESSTATUS_FAILED, error_string(GetLastError()));
		return 0;
	}

	ENUM_SERVICE_STATUS_PROCESS* status = (ENUM_SERVICE_STATUS_PROCESS*)HeapAlloc(GetProcessHeap(), 0, required);
	if (!status) {
		print_message(stderr, FCSM_MESSAGE_OUT_OF_MEMORY, _T("ENUM_SERVICE_STATUS_PROCESS"), _T("open_service()"));
		return 0;
	}
		...
		for (i = 0; i < count; i++) {
			if (str_equiv(status[i].lpDisplayName, service_name)) {
				if (_sntprintf_s(canonical_name, canonical_namelen, _TRUNCATE, _T("%s"), status[i].lpServiceName) < 0) {
					HeapFree(GetProcessHeap(), 0, status);
					print_message(stderr, FCSM_MESSAGE_OUT_OF_MEMORY, _T("canonical_name"), _T("open_service()"));
					return 0;
				}
...
QUERY_SERVICE_CONFIG* query_service_config(const TCHAR* service_name, SC_HANDLE service_handle) {
	...
	if (error == ERROR_INSUFFICIENT_BUFFER) {
		...
		if (!qsc) {
			print_message(stderr, FCSM_MESSAGE_OUT_OF_MEMORY, _T("QUERY_SERVICE_CONFIG"), _T("query_service_config()"), 0);
			return 0;
		}
	}
	else {
		print_message(stderr, FCSM_MESSAGE_QUERYSERVICECONFIG_FAILED, service_name, error_string(error), 0);
		return 0;
	}

	if (!QueryServiceConfig(service_handle, qsc, bufsize, &bufsize)) {
		HeapFree(GetProcessHeap(), 0, qsc);
		print_message(stderr, FCSM_MESSAGE_QUERYSERVICECONFIG_FAILED, service_name, error_string(GetLastError()), 0);
		return 0;
	}
...
int prepend_service_group_identifier(TCHAR* group, TCHAR** canon) {
	...
	*canon = (TCHAR*)HeapAlloc(GetProcessHeap(), 0, (len + 1) * sizeof(TCHAR));
	if (!*canon) {
		print_message(stderr, FCSM_MESSAGE_OUT_OF_MEMORY, _T("canon"), _T("prepend_service_group_identifier()"));
		return 1;
	}
...
int set_service_dependencies(const TCHAR* service_name, SC_HANDLE service_handle, TCHAR* buffer) {
	...
	if (buffer && buffer[0]) {
		...
		if (!services) {
			print_message(stderr, FCSM_MESSAGE_OPEN_SERVICE_MANAGER_FAILED);
			return 1;
		}
		...
		*/
		...
		if (groups) {
			HKEY key;
			if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, FCSM_REGISTRY_GROUPS, 0, KEY_READ, &key)) {
				_ftprintf(stderr, _T("%s: %s\n"), FCSM_REGISTRY_GROUPS, error_string(GetLastError()));
				return 2;
			}

			unsigned long type;
			unsigned long groupslen;
			unsigned long ret = RegQueryValueEx(key, FCSM_REG_GROUPS, 0, &type, NULL, &groupslen);
			if (ret == ERROR_SUCCESS) {
				groups = (TCHAR*)HeapAlloc(GetProcessHeap(), 0, groupslen);
				if (!groups) {
					print_message(stderr, FCSM_MESSAGE_OUT_OF_MEMORY, _T("groups"), _T("set_service_dependencies()"));
					return 3;
				}

				ret = RegQueryValueEx(key, FCSM_REG_GROUPS, 0, &type, (unsigned char*)groups, &groupslen);
				if (ret != ERROR_SUCCESS) {
					_ftprintf(stderr, _T("%s\\%s: %s"), FCSM_REGISTRY_GROUPS, FCSM_REG_GROUPS, error_string(GetLastError()));
	...
	if (!ChangeServiceConfig(service_handle, SERVICE_NO_CHANGE, SERVICE_NO_CHANGE, SERVICE_NO_CHANGE, 0, 0, 0, dependencies, 0, 0, 0)) {
		...
		print_message(stderr, FCSM_MESSAGE_CHANGESERVICECONFIG_FAILED, error_string(GetLastError()));
		return -1;
	}
...
int get_service_dependencies(const TCHAR* service_name, SC_HANDLE service_handle, TCHAR** buffer, unsigned long* bufsize, int type) {
	...
	*buffer = (TCHAR*)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, *bufsize * sizeof(TCHAR));
	if (!*buffer) {
		*bufsize = 0;
		print_message(stderr, FCSM_MESSAGE_OUT_OF_MEMORY, _T("lpDependencies"), _T("get_service_dependencies()"));
		HeapFree(GetProcessHeap(), 0, qsc);
		return 4;
	}
...
int set_service_description(const TCHAR* service_name, SC_HANDLE service_handle, TCHAR* buffer) {
	...
	*/
	...
	log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_SERVICE_CONFIG_DESCRIPTION_FAILED, service_name, error_string(GetLastError()), 0);
	return 1;
}
...
int get_service_description(const TCHAR* service_name, SC_HANDLE service_handle, unsigned long len, TCHAR* buffer) {
	...
	if (error == ERROR_INSUFFICIENT_BUFFER) {
		...
		if (!description) {
			print_message(stderr, FCSM_MESSAGE_OUT_OF_MEMORY, _T("SERVICE_CONFIG_DESCRIPTION"), _T("get_service_description()"));
			return 2;
		}
		...
		if (QueryServiceConfig2(service_handle, SERVICE_CONFIG_DESCRIPTION, (unsigned char*)description, bufsize, &bufsize)) {
			...
			HeapFree(GetProcessHeap(), 0, description);
			print_message(stderr, FCSM_MESSAGE_QUERYSERVICECONFIG2_FAILED, service_name, _T("SERVICE_CONFIG_DESCRIPTION"), error_string(error));
			return 3;
		}
	}
	else {
...
int get_service_startup(const TCHAR* service_name, SC_HANDLE service_handle, const QUERY_SERVICE_CONFIG* qsc, unsigned long* startup) {
	...
	case SERVICE_DEMAND_START: *startup = FCSM_STARTUP_MANUAL; break;
	case SERVICE_DISABLED: *startup = FCSM_STARTUP_DISABLED; break;
	default: *startup = FCSM_STARTUP_AUTOMATIC;
	}

	if (*startup != FCSM_STARTUP_AUTOMATIC) return 0;
	...
	if (error == ERROR_INSUFFICIENT_BUFFER) {
		...
		if (!info) {
			print_message(stderr, FCSM_MESSAGE_OUT_OF_MEMORY, _T("SERVICE_DELAYED_AUTO_START_INFO"), _T("get_service_startup()"));
			return 2;
		}

		if (QueryServiceConfig2(service_handle, SERVICE_CONFIG_DELAYED_AUTO_START_INFO, (unsigned char*)info, bufsize, &bufsize)) {
			if (info->fDelayedAutostart) *startup = FCSM_STARTUP_DELAYED;
			...
			if (error != ERROR_INVALID_LEVEL) {
				print_message(stderr, FCSM_MESSAGE_QUERYSERVICECONFIG2_FAILED, service_name, _T("SERVICE_CONFIG_DELAYED_AUTO_START_INFO"), error_string(error));
				return 3;
			}
		}
	}
	else if (error != ERROR_INVALID_LEVEL) {
		print_message(stderr, FCSM_MESSAGE_QUERYSERVICECONFIG2_FAILED, service_name, _T("SERVICE_DELAYED_AUTO_START_INFO"), error_string(error));
		return 3;
	}
	...
	*usernamelen = 0;
	...
	if (qsc->lpServiceStartName[0]) {
		...
		*username = (TCHAR*)HeapAlloc(GetProcessHeap(), 0, (len + 1) * sizeof(TCHAR));
		if (!*username) {
			print_message(stderr, FCSM_MESSAGE_OUT_OF_MEMORY, _T("username"), _T("get_service_username()"));
			return 2;
		}
...
void set_fcsm_service_defaults(fcsm_service_t* service) {
	if (!service) return;

	service->type = SERVICE_WIN32_OWN_PROCESS;
	service->priority = NORMAL_PRIORITY_CLASS;
	service->stdin_sharing = FCSM_STDIN_SHARING;
	service->stdin_disposition = FCSM_STDIN_DISPOSITION;
	service->stdin_flags = FCSM_STDIN_FLAGS;
	service->stdout_sharing = FCSM_STDOUT_SHARING;
	service->stdout_disposition = FCSM_STDOUT_DISPOSITION;
	service->stdout_flags = FCSM_STDOUT_FLAGS;
	service->stderr_sharing = FCSM_STDERR_SHARING;
	service->stderr_disposition = FCSM_STDERR_DISPOSITION;
	service->stderr_flags = FCSM_STDERR_FLAGS;
	service->throttle_delay = FCSM_RESET_THROTTLE_RESTART;
	service->stop_method = ~0;
	service->kill_console_delay = FCSM_KILL_CONSOLE_GRACE_PERIOD;
	service->kill_window_delay = FCSM_KILL_WINDOW_GRACE_PERIOD;
	service->kill_threads_delay = FCSM_KILL_THREADS_GRACE_PERIOD;
	service->kill_process_tree = 1;
}

/* Allocate and zero memory for a service. */
fcsm_service_t* alloc_fcsm_service() {
	fcsm_service_t* service = (fcsm_service_t*)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(fcsm_service_t));
	if (!service) log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, _T("service"), _T("alloc_fcsm_service()"), 0);
	return service;
}

/* Free memory for a service. */
void cleanup_fcsm_service(fcsm_service_t* service) {
	if (!service) return;
...
int pre_install_service(int argc, TCHAR** argv) {
	...
	if (!service) {
		print_message(stderr, FCSM_MESSAGE_OUT_OF_MEMORY, _T("service"), _T("pre_install_service()"));
		return 1;
	}
	...
	if (flagslen > _countof(service->flags)) {
		print_message(stderr, FCSM_MESSAGE_FLAGS_TOO_LONG);
		return 2;
	}
...
int pre_edit_service(int argc, TCHAR** argv) {
	...
	if (mandatory > 2) {
		...
		if (!settings[i].name) {
			print_message(stderr, FCSM_MESSAGE_INVALID_PARAMETER, parameter);
		...
		if (additional_mandatory) {
			if (argc < mandatory) {
				print_message(stderr, FCSM_MESSAGE_MISSING_SUBPARAMETER, parameter);
				return 1;
			}
			additional = argv[3];
			remainder = 4;
		}
		else if (str_equiv(setting->name, FCSM_NATIVE_OBJECTNAME) && mode == MODE_SETTING) {
			additional = argv[3];
			remainder = 4;
		}
		else {
	...
	if (!(service->type & SERVICE_WIN32_OWN_PROCESS)) {
		if (mode != MODE_GETTING && mode != MODE_DUMPING) {
			...
			CloseServiceHandle(service->handle);
			CloseServiceHandle(services);
			print_message(stderr, FCSM_MESSAGE_CANNOT_EDIT, service->name, FCSM_WIN32_OWN_PROCESS, 0);
			return 3;
		}
	}
	...
	/* Remember the executable in case it isn't FCSM. */
	...
	if (!service->exe[0]) {
		service->native = true;
		if (mode != MODE_GETTING && mode != MODE_DUMPING) print_message(stderr, FCSM_MESSAGE_INVALID_SERVICE, service->name, FCSM, service->image);
	}
	...
	if (mode == MODE_DUMPING) {
		...
		TCHAR quoted_fcsm[EXE_LENGTH * 2];
		...
		if (quote(fcsm_exe(), quoted_fcsm, _countof(quoted_fcsm))) return 6;
		_tprintf(_T("%s install %s %s\n"), quoted_fcsm, quoted_service_name, quoted_exe);
	...
	/* Trying to manage App* parameters for a non-FCSM service. */
	if (!setting->native && service->native) {
		CloseServiceHandle(service->handle);
		print_message(stderr, FCSM_MESSAGE_NATIVE_PARAMETER, setting->name, FCSM);
		return 1;
	}
	...
	if (mode == MODE_RESETTING) {
		...
		if (!value.string) {
			print_message(stderr, FCSM_MESSAGE_OUT_OF_MEMORY, _T("value"), _T("edit_service()"));
			CloseServiceHandle(service->handle);
			return 2;
		}
...
int pre_remove_service(int argc, TCHAR** argv) {
	...
	print_message(stderr, FCSM_MESSAGE_PRE_REMOVE_SERVICE);
	return 100;
}
...
int install_service(fcsm_service_t* service, bool editing) {
	...
	_sntprintf_s(service->image, _countof(service->image), _TRUNCATE, _T("%s"), fcsm_imagepath());
	...
	if (!service->handle) {
		print_message(stderr, FCSM_MESSAGE_CREATESERVICE_FAILED, error_string(GetLastError()));
		CloseServiceHandle(services);
		return 5;
	}
	...
	print_message(stdout, FCSM_MESSAGE_SERVICE_INSTALLED, service->name);
...
/* Edit the service. */
int edit_service(fcsm_service_t* service, bool editing) {
	if (!service) return 1;
	...
	*/
	...
	case FCSM_STARTUP_MANUAL: startup = SERVICE_DEMAND_START; break;
	case FCSM_STARTUP_DISABLED: startup = SERVICE_DISABLED; break;
	...
	*/
	...
	if (service->usernamelen) {
		...
		if (is_virtual_account(service->name, username)) {
			...
			if (!canon) {
				print_message(stderr, FCSM_MESSAGE_OUT_OF_MEMORY, _T("canon"), _T("edit_service()"));
				return 5;
			}
	...
	else if (editing) username = canon = FCSM_LOCALSYSTEM_ACCOUNT;
	...
	if (!virtual_account) {
		if (well_known_username(canon)) password = _T("");
			...
			if (grant_logon_as_service(canon)) {
				...
				print_message(stderr, FCSM_MESSAGE_GRANT_LOGON_AS_SERVICE_FAILED, username);
				return 5;
			}
		}
	}
	...
	if (service->startup == FCSM_STARTUP_DELAYED) delayed.fDelayedAutostart = 1;
	...
	if (!ChangeServiceConfig2(service->handle, SERVICE_CONFIG_DELAYED_AUTO_START_INFO, &delayed)) {
		...
		if (error != ERROR_INVALID_LEVEL) {
			log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_SERVICE_CONFIG_DELAYED_AUTO_START_INFO_FAILED, service->name, error_string(error), 0);
		}
	}
	...
	if (!service->native) {
		...
		if (create_parameters(service, editing)) {
			print_message(stderr, FCSM_MESSAGE_CREATE_PARAMETERS_FAILED);
			return 6;
		}
...
int control_service(unsigned long control, int argc, TCHAR** argv, bool return_status) {
	...
	if (control == FCSM_SERVICE_CONTROL_START) {
		...
		if (ret) {
			...
			if (key) {
				if (get_number(key, FCSM_REG_THROTTLE, &cutoff, false) != 1) cutoff = FCSM_RESET_THROTTLE_RESTART;
				RegCloseKey(key);
			}
			...
			if (response) {
				print_message(stderr, FCSM_MESSAGE_BAD_CONTROL_RESPONSE, canonical_name, service_status_text(service_status.dwCurrentState), service_control_text(control));
				if (return_status) return 0;
				return 1;
			}
...
/* Remove the service */
int remove_service(fcsm_service_t* service) {
	if (!service) return 1;
	...
	if (!DeleteService(service->handle)) {
		print_message(stderr, FCSM_MESSAGE_DELETESERVICE_FAILED);
		CloseServiceHandle(services);
		return 4;
	}
	...
	print_message(stdout, FCSM_MESSAGE_SERVICE_REMOVED, service->name);
	return 0;
}
...
void WINAPI service_main(unsigned long argc, TCHAR** argv) {
	...
	if (_sntprintf_s(service->name, _countof(service->name), _TRUNCATE, _T("%s"), argv[0]) < 0) {
		log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, _T("service->name"), _T("service_main()"), 0);
		return;
	}
	...
	service->status.dwWaitHint = FCSM_WAITHINT_MARGIN;
	...
	service->status_handle = RegisterServiceCtrlHandlerEx(FCSM, service_control_handler, (void*)service);
	if (!service->status_handle) {
		log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_REGISTERSERVICECTRLHANDER_FAILED, error_string(GetLastError()), 0);
		return;
	}
	...
	service->status.dwWaitHint = service->throttle_delay + FCSM_WAITHINT_MARGIN;
	...
	if (use_critical_section) {
		...
		if (!service->throttle_timer) {
			log_event(EVENTLOG_WARNING_TYPE, FCSM_EVENT_CREATEWAITABLETIMER_FAILED, service->name, error_string(GetLastError()), 0);
		}
	}
	...
	if (get_process_creation_time(GetCurrentProcess(), &service->fcsm_creation_time)) ZeroMemory(&service->fcsm_creation_time, sizeof(service->fcsm_creation_time));

	service->allow_restart = true;
	if (!CreateThread(NULL, 0, launch_service, (void*)service, 0, NULL)) {
		log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_CREATETHREAD_FAILED, error_string(GetLastError()), 0);
		stop_service(service, 0, true, true);
	}
}

/* Make sure service recovery actions are taken where necessary */
void set_service_recovery(fcsm_service_t* service) {
	...
	if (!ChangeServiceConfig2(service->handle, SERVICE_CONFIG_FAILURE_ACTIONS_FLAG, &flag)) {
		...
		/* Pre-Vista we expect to fail with ERROR_INVALID_LEVEL */
		if (error != ERROR_INVALID_LEVEL) {
			log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_SERVICE_CONFIG_FAILURE_ACTIONS_FAILED, service->name, error_string(error), 0);
		}
	}
}

int monitor_service(fcsm_service_t* service) {
	/* Set service status to started */
	int ret = start_service(service);
	if (ret) {
		TCHAR code[16];
		_sntprintf_s(code, _countof(code), _TRUNCATE, _T("%d"), ret);
		log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_START_SERVICE_FAILED, service->exe, service->name, ret, 0);
		return ret;
	}
	log_event(EVENTLOG_INFORMATION_TYPE, FCSM_EVENT_STARTED_SERVICE, service->exe, service->flags, service->name, service->dir, 0);
	...
	if (!RegisterWaitForSingleObject(&service->wait_handle, service->process_handle, end_service, (void*)service, INFINITE, WT_EXECUTEONLYONCE | WT_EXECUTELONGFUNCTION)) {
		log_event(EVENTLOG_WARNING_TYPE, FCSM_EVENT_REGISTERWAITFORSINGLEOBJECT_FAILED, service->name, service->exe, error_string(GetLastError()), 0);
	}
...
TCHAR* service_control_text(unsigned long control) {
	...
	case FCSM_SERVICE_CONTROL_START: return _T("START");
	...
	case FCSM_SERVICE_CONTROL_ROTATE: return _T("ROTATE");
...
void log_service_control(TCHAR* service_name, unsigned long control, bool handled) {
	...
	if (!text) {
		...
		if (!text) {
			log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, _T("control code"), _T("log_service_control()"), 0);
			return;
		}
		...
		event = FCSM_EVENT_SERVICE_CONTROL_UNKNOWN;
	}
	else if (handled) event = FCSM_EVENT_SERVICE_CONTROL_HANDLED;
	else event = FCSM_EVENT_SERVICE_CONTROL_NOT_HANDLED;

	log_event(EVENTLOG_INFORMATION_TYPE, event, service_name, text, 0);

	if (event == FCSM_EVENT_SERVICE_CONTROL_UNKNOWN) {
		HeapFree(GetProcessHeap(), 0, text);
	}
}
...
unsigned long WINAPI service_control_handler(unsigned long control, unsigned long event, void* data, void* context) {
	fcsm_service_t* service = (fcsm_service_t*)context;
	...
	case SERVICE_CONTROL_STOP:
		...
		/* Pre-stop hook. */
		fcsm_hook(&hook_threads, service, FCSM_HOOK_EVENT_STOP, FCSM_HOOK_ACTION_PRE, &control, FCSM_SERVICE_STATUS_DEADLINE, false);
	...
	case SERVICE_CONTROL_CONTINUE:
		...
		service->status.dwWaitHint = throttle_milliseconds(service->throttle) + FCSM_WAITHINT_MARGIN;
		log_event(EVENTLOG_INFORMATION_TYPE, FCSM_EVENT_RESET_THROTTLE, service->name, 0);
		SetServiceStatus(service->status_handle, &service->status);
		return NO_ERROR;
	...
	case FCSM_SERVICE_CONTROL_ROTATE:
		service->last_control = control;
		log_service_control(service->name, control, true);
		(void)fcsm_hook(&hook_threads, service, FCSM_HOOK_EVENT_ROTATE, FCSM_HOOK_ACTION_PRE, &control, FCSM_HOOK_DEADLINE, false);
		if (service->rotate_stdout_online == FCSM_ROTATE_ONLINE) service->rotate_stdout_online = FCSM_ROTATE_ONLINE_ASAP;
		if (service->rotate_stderr_online == FCSM_ROTATE_ONLINE) service->rotate_stderr_online = FCSM_ROTATE_ONLINE_ASAP;
		(void)fcsm_hook(&hook_threads, service, FCSM_HOOK_EVENT_ROTATE, FCSM_HOOK_ACTION_POST, &control);
		return NO_ERROR;
	...
	case SERVICE_CONTROL_POWEREVENT:
		...
		if (event == PBT_APMRESUMEAUTOMATIC) {
			service->last_control = control;
			log_service_control(service->name, control, true);
			(void)fcsm_hook(&hook_threads, service, FCSM_HOOK_EVENT_POWER, FCSM_HOOK_ACTION_RESUME, &control);
			return NO_ERROR;
		}
		...
		if (event == PBT_APMPOWERSTATUSCHANGE) {
			service->last_control = control;
			log_service_control(service->name, control, true);
			(void)fcsm_hook(&hook_threads, service, FCSM_HOOK_EVENT_POWER, FCSM_HOOK_ACTION_CHANGE, &control);
			return NO_ERROR;
		}
		log_service_control(service->name, control, false);
		return NO_ERROR;
...
/* Start the service */
int start_service(fcsm_service_t* service) {
	...
	if (ret) {
		log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_GET_PARAMETERS_FAILED, service->name, 0);
	...
	if (_sntprintf_s(cmd, _countof(cmd), _TRUNCATE, _T("\"%s\" %s"), service->exe, service->flags) < 0) {
		log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_OUT_OF_MEMORY, _T("command line"), _T("start_service"), 0);
		unset_service_environment(service);
		return stop_service(service, 2, true, true);
	}
	...
	unsigned long control = FCSM_SERVICE_CONTROL_START;
	...
	if (service->allow_restart) {
		...
		if (fcsm_hook(&hook_threads, service, FCSM_HOOK_EVENT_START, FCSM_HOOK_ACTION_PRE, &control, FCSM_SERVICE_STATUS_DEADLINE, false) == FCSM_HOOK_STATUS_ABORT) {
			TCHAR code[16];
			_sntprintf_s(code, _countof(code), _TRUNCATE, _T("%lu"), FCSM_HOOK_STATUS_ABORT);
			log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_PRESTART_HOOK_ABORT, FCSM_HOOK_EVENT_START, FCSM_HOOK_ACTION_PRE, service->name, code, 0);
			unset_service_environment(service);
		...
		if (!CreateProcess(0, cmd, 0, 0, inherit_handles, flags, 0, service->dir, &si, &pi)) {
			...
			unsigned long error = GetLastError();
			log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_CREATEPROCESS_FAILED, service->name, service->exe, error_string(error), 0);
			close_output_handles(&si);
			unset_service_environment(service);
		...
		if (service->affinity) {
			/*
			  ...
			  (or when running the 32-bit FCSM).
			...
			*/
			...
			if (GetProcessAffinityMask(service->process_handle, &affinity, &system_affinity)) affinity = service->affinity & system_affinity;
				...
				log_event(EVENTLOG_ERROR_TYPE, FCSM_EVENT_GETPROCESSAFFINITYMASK_FAILED, service->name, error_string(GetLastError()), 0);
			}

			if (!SetProcessAffinityMask(service->process_handle, affinity)) {
				log_event(EVENTLOG_WARNING_TYPE, FCSM_EVENT_SETPROCESSAFFINITYMASK_FAILED, service->name, error_string(GetLastError()), 0);
			}
	...
	*/
	...
	/* Post-start hook. */
	if (!service->throttle) {
		(void)fcsm_hook(&hook_threads, service, FCSM_HOOK_EVENT_START, FCSM_HOOK_ACTION_POST, &control);
	}
...
/* Stop the service */
int stop_service(fcsm_service_t* service, unsigned long exitcode, bool graceful, bool default_action) {
	service->allow_restart = false;
	...
	service->rotate_stdout_online = service->rotate_stderr_online = FCSM_ROTATE_OFFLINE;

	if (default_action && !exitcode && !graceful) {
		log_event(EVENTLOG_INFORMATION_TYPE, FCSM_EVENT_GRACEFUL_SUICIDE, service->name, service->exe, exit_action_strings[FCSM_EXIT_UNCLEAN], exit_action_strings[FCSM_EXIT_UNCLEAN], exit_action_strings[FCSM_EXIT_UNCLEAN], exit_action_strings[FCSM_EXIT_REALLY], 0);
		graceful = true;
	}
	...
	if (service->pid) {
		/* Shut down service */
		log_event(EVENTLOG_INFORMATION_TYPE, FCSM_EVENT_TERMINATEPROCESS, service->name, service->exe, 0);
		kill_t k;
		service_kill_t(service, &k);
		k.exitcode = 0;
		kill_process(&k);
	}
	else log_event(EVENTLOG_INFORMATION_TYPE, FCSM_EVENT_PROCESS_ALREADY_STOPPED, service->name, service->exe, 0);
...
void CALLBACK end_service(void* arg, unsigned char why) {
	fcsm_service_t* service = (fcsm_service_t*)arg;
	...
	*/
	if (!why) {
		...
		log_event(EVENTLOG_INFORMATION_TYPE, FCSM_EVENT_ENDED_SERVICE, service->exe, service->name, code, 0);
	}
	...
	/* Exit hook. */
	service->exit_count++;
	(void)fcsm_hook(&hook_threads, service, FCSM_HOOK_EVENT_EXIT, FCSM_HOOK_ACTION_POST, NULL, FCSM_HOOK_DEADLINE, true);
	...
	*/
	...
	case FCSM_EXIT_RESTART:
		log_event(EVENTLOG_INFORMATION_TYPE, FCSM_EVENT_EXIT_RESTART, service->name, code, exit_action_strings[action], service->exe, 0);
		while (monitor_service(service)) {
			log_event(EVENTLOG_WARNING_TYPE, FCSM_EVENT_RESTART_SERVICE_FAILED, service->exe, service->name, 0);
			Sleep(30000);
		}
		break;

		/* Do nothing, just like srvany would */
	case FCSM_EXIT_IGNORE:
		log_event(EVENTLOG_INFORMATION_TYPE, FCSM_EVENT_EXIT_IGNORE, service->name, code, exit_action_strings[action], service->exe, 0);
		wait_for_hooks(service, false);
		Sleep(INFINITE);
		break;

		/* Tell the service manager we are finished */
	case FCSM_EXIT_REALLY:
		log_event(EVENTLOG_INFORMATION_TYPE, FCSM_EVENT_EXIT_REALLY, service->name, code, exit_action_strings[action], 0);
		stop_service(service, exitcode, true, default_action);
		break;

		/* Fake a crash so pre-Vista service managers will run recovery actions. */
	case FCSM_EXIT_UNCLEAN:
		log_event(EVENTLOG_INFORMATION_TYPE, FCSM_EVENT_EXIT_UNCLEAN, service->name, code, exit_action_strings[action], 0);
...
void throttle_restart(fcsm_service_t* service) {
	...
	if (service->throttle == 1 && service->restart_delay > throttle_ms) log_event(EVENTLOG_INFORMATION_TYPE, FCSM_EVENT_RESTART_DELAY, service->name, milliseconds, 0);
	else {
		_sntprintf_s(threshold, _countof(threshold), _TRUNCATE, _T("%lu"), service->throttle_delay);
		log_event(EVENTLOG_WARNING_TYPE, FCSM_EVENT_THROTTLED, service->name, threshold, milliseconds, 0);
	}
...
/*
  ...
  FCSM_SERVICE_STATUS_DEADLINE milliseconds.  If the process is still running
  and we haven't finished waiting we increment dwCheckPoint and add whichever is
  smaller of FCSM_SERVICE_STATUS_DEADLINE or the remaining timeout to
  dwWaitHint.
...
int await_single_handle(SERVICE_STATUS_HANDLE status_handle, SERVICE_STATUS* status, HANDLE handle, TCHAR* name, TCHAR* function_name, unsigned long timeout) {
	...
	while (waited < timeout) {
		...
		if (interval > FCSM_SERVICE_STATUS_DEADLINE) interval = FCSM_SERVICE_STATUS_DEADLINE;
		...
		if (waited) {
			...
			log_event(EVENTLOG_INFORMATION_TYPE, FCSM_EVENT_AWAITING_SINGLE_HANDLE, function, name, waited_milliseconds, interval_milliseconds, timeout_milliseconds, 0);
		}
...
int list_fcsm_services(int argc, TCHAR** argv) {
	...
	ENUM_SERVICE_STATUS_PROCESS* status = (ENUM_SERVICE_STATUS_PROCESS*)HeapAlloc(GetProcessHeap(), 0, required);
	if (!status) {
		print_message(stderr, FCSM_MESSAGE_OUT_OF_MEMORY, _T("ENUM_SERVICE_STATUS_PROCESS"), _T("list_fcsm_services()"));
		return 3;
	}
	...
	while (true) {
		...
		for (i = 0; i < count; i++) {
			...
			if (!service) {
				HeapFree(GetProcessHeap(), 0, status);
				print_message(stderr, FCSM_MESSAGE_OUT_OF_MEMORY, _T("fcsm_service_t"), _T("list_fcsm_services()"));
				return 5;
			}
